/* ============================================================
   DOG API — DOG API 外设总线内核 (v1)
   ------------------------------------------------------------
   本文件是「§4.7 外设总线」的计算内核，与 assets/js/quantify.js 并列：
...
(function (root, factory) {
  ...
  var ROLES = [
    ...
    { key: "constraint", label: "约束器", note: "不承接配额，只限制爆发性能（§2.9 的超频）" },
    { key: "suppressor", label: "抑制器", note: "不承接配额，只降低某一项风险或唤醒指标" },
    { key: "deprecated", label: "已弃用", note: "序列不可闭合。规范组不建议部署，见 §4.7 弃用清单" }
  ];
  ...
  var CLASSES = [
    ...
    },
    {
      ...
      driver: "藏食 → 鼻主导搜寻",
      intent: "唯一直接承接嗅闻维度的外设。配合 §4.1 的 1:3 换算，它是把「运动时间」换成「嗅闻时间」的最低成本手段。",
      ...
      src: "嗅闻为独立动机系统（见 §1.4 嗅觉阵列、§4.1 嗅闻—奔跑当量比）"
    },
    {
    ...
    },
    {
      ...
      driver: "约束爆发性能（§2.10 的 +300% 松鼠超频）",
      intent: "本类不承接配额，它约束的是 §2.9 里那条超频。它同时是本规范中唯一一类「默认由你承担全部事故责任」的外设。",
    ...
    },
    {
      ...
      gates: ["material"],
      faults: [
        { mode: "占据决策位置", why: "冷却垫会让人以为「有垫子就能出门」。热射病的处置窗口是分钟级（§4.4 SCN-E2）", fix: "垫子是缓解，不是许可。硬停条款优先" },
        { mode: "被啃食（凝胶款）", why: "凝胶垫一旦被咬破，内容物即为外来体风险", fix: "只在不被啃咬的场景下使用，或选不可啃穿的硬壳款" }
      ],
      src: "本项目 HRI 模型（§4.5 C 级复合指标）与散热架构（§1.4 外泌汗腺仅爪垫）"
    },
    {
  ...
  function checkDevice(dev, spec, profile) {
    ...
    c.gates.forEach(function (k) {
      if (k === "airway") {
        ...
        row(k, "rule", "伸缩绳不在推荐配置内。候诊区必须用固定长度牵引（§4.2）");
      }
    });
  ...
  function lineup(devices, profile, env, act, flags) {
    ...
    return {
      ...
      shortCircuitNote: shortCircuit
        ? "分诊结论为 " + tri.code + "。按 §4.5，其余接口在此状态下不再有意义 —— 外设清单同样不例外。先开车。"

...
> 本文是 §30《表情接口定义（Expression API v1）》的工程说明。它回答一个问题：**那一帧像素，是怎么从一组数字变成图形的。**
...
这是本模块存在的理由。这里的「链接」不是比喻，而是已实测的耦合通道（正文 §24 / §30）：
...
（`data-state` / `aria-label` 按设计就是不同的）。这条断言是 §30 的核心声明，它被破坏时不会有任何别的症状。
